// returns the more recent version between two
// example of versions : "1.0.9", "1", "1.2"
function getRecentVersion(s1, s2){
    var v1 = s1.split('.'), v2 = s2.split('.');
    ...
    return (v1.length>=v2.length) ? s1 : s2;
}

// function that add a js script and then go to callback
function require(url, condition, callback){
...
// contains all the javascript for the plugin
function getJS(jq){
    (function($) {
    ...
    (function($){
        $('a.tooltip').cluetip({
            ...
            onShow: function(ct, ci){
                ...
                $('#tooltip_div').html('<iframe id="tooltip_iframe" src="'+url+'" border="0" frameborder="0" scrolling="no" style="background:#fafafa;width:100%;height:350px;border:0;text-align:center"></iframe>');
            }
        });
    })(jq);
}

// minimum jQuery version for the plugin to work
var pluginVersion = "1.4.0";

// current wordpress jquery version is more recent
if(jQuery && getRecentVersion(jQuery.fn.jquery, pluginVersion) == jQuery.fn.jquery){
    // add the javascript plugin using this jquery instance
    getJS(jQuery);
}
// add the jquery recommended version (with no conflict)
else {
    require('http://ajax.googleapis.com/ajax/libs/jquery/'+pluginVersion+'/jquery.min.js', true, function(){
        window.jQueryDT = jQuery.noConflict();
        // make the javascript plugun use this no-conlict jquery version.
        getJS(jQueryDT);
    });
}
